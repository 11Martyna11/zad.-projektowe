...
def launch_main_app() -> None:
    ...
    def add_store():
        ...
        def _finish(coords):
            if coords is None:
                messagebox.showerror("Geokoder", f"Adres „{addr}” nie znaleziony.")
                return
    ...
    def edit_store():
        ...
        win = tk.Toplevel(app);
        win.title("Edytuj sklep")
        ttk.Label(win, text="Nazwa:").grid(row=0, column=0, sticky="e")
        name_ent = tk.Entry(win, width=30);
        name_ent.grid(row=0, column=1)
        ttk.Label(win, text="Adres:").grid(row=1, column=0, sticky="e")
        addr_ent = tk.Entry(win, width=30);
        addr_ent.grid(row=1, column=1);
        addr_ent.insert(0, st.address)
        ...
        def _save():
            ...
            def _finish(coords):
                if coords is None:
                    messagebox.showerror("Geokoder", f"Adres „{new_addr}” nie znaleziony.")
                    return
    ...
    def del_emp():
        if not employee_lb.curselection():
            return
        flt, sel_idx = emp_filter_cmb.get(), employee_lb.curselection()[0]
    ...
    def edit_emp():
        if not employee_lb.curselection():
            return
        flt, sel_idx = emp_filter_cmb.get(), employee_lb.curselection()[0]
        e = employees[sel_idx] if flt == "– Wszystkie –" else (
            next(s for s in stores if str(s) == flt).employees[sel_idx])
        ...
        cmb.set(str(e.store) if e.store else "(brak)")

        def _save():
            e.fullname, e.position = name_ent.get().strip(), pos_ent.get().strip()
            new_loc = name_ent.get().strip()
            ...
            if new_loc != e.location:
                e.location = new_loc;
                e.lat, e.lon = wikigeocode(new_loc)
            refresh_emp_lb();
            refresh_map();
            win.destroy()
    ...
    def edit_sup():
        ...
        s = suppliers[idx] if flt == "– Wszystkie –" else (
            next(st for st in stores if str(st) == flt).suppliers[sel_idx])

        win = tk.Toplevel(app);
        win.title("Edytuj dostawcę")
        ...
        cmb.grid(row=3, column=1);
        cmb.set(str(s.store) if s.store else "(brak)")
    ...
    def _save():
        ...
        if new_loc != s.location:
            s.location = new_loc;
            s.lat, s.lon = wikigeocode(new_loc)
        refresh_sup_lb();
        refresh_map();
        win.destroy()
    ...
    emp_assign_cmb.grid(row=3, column=1)
    emp_assign_cmb.set("(brak)")
