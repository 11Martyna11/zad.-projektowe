...
class Store:
    def __init__(self, name: str, loc: str):
        ...
        self.lat, self.lon = wikigeocode(loc)
...
class Employee:
    def __init__(self, full: str, pos: str, loc: str, store: Store | None = None):
        self.fullname, self.position, self.location = full, pos, loc
        self.lat, self.lon = wikigeocode(loc)
        self.store = store
        self.marker = None
...
class Supplier:
    def __init__(self, name: str, cat: str, loc: str, store: Store | None = None):
        self.name, self.category, self.location = name, cat, loc
        self.lat, self.lon = wikigeocode(loc)
        self.store = store
        self.marker = None
...
def launch_main_app() -> None:
    ...
    # --------------- CRUD sklepy -----------------------
    def add_store():
        n, loc = store_name_ent.get().strip(), store_loc_ent.get().strip()
        if not n or not loc:
            return
        st = Store(n, loc)
        stores.append(st)
    ...
    ttk.Label(frm_s, text="Miejscowość:").grid(row=1, column=0, sticky="e")
...
login_win.geometry("380x160")
ttk.Label(login_win, text="Użytkownik:").pack(pady=4)
username_entry = ttk.Entry(login_win); username_entry.pack()
ttk.Label(login_win, text="Hasło:").pack(pady=4)
password_entry = ttk.Entry(login_win, show="*"); password_entry.pack()
ttk.Button(login_win, text="Zaloguj", command=attempt_login).pack(pady=12)
login_win.mainloop()